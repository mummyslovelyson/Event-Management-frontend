  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <button onClick={() => setInviteModal(true)} className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg bg-[#D4AF37] text-[#1E252B] text-sm font-semibold hover:bg-[#c4a030] transition-colors shrink-0">
            ...
            <div key={role} className="rounded-xl bg-[#171A1D] border border-[#262B2F] p-5">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  ...
                  'bg-[#494F55]/30 text-[#8A9196]'
                }`}>
                  <Icon className="w-5 h-5" />
                </div>
                <h3 className="text-base font-semibold text-[#EDF0F1]">{role}</h3>
              </div>
              <p className="mt-3 text-sm text-[#8A9196] leading-relaxed">{info.desc}</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {info.perms.map((p) => (
                  <span key={p} className="px-2 py-0.5 rounded text-[10px] font-medium bg-[#494F55]/20 text-[#8A9196] border border-[#262B2F]">
                    {p}
                  </span>
                ))}
              </div>
        ...
        <>
          ...
          <section className="space-y-3">
            <h2 className="text-lg font-semibold text-[#EDF0F1] flex items-center gap-2">
              ...
              <span className="text-sm text-[#8A9196] font-normal">({members.length})</span>
            </h2>
            {members.length === 0 ? (
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-[#EDF0F1] truncate">{m.name || m.email}</p>
                              <p className="text-xs text-[#8A9196] truncate">{m.email}</p>
                              {m.dateAdded && (
          ...
          {invites.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-lg font-semibold text-[#EDF0F1] flex items-center gap-2">
                ...
                <span className="text-sm text-[#8A9196] font-normal">({invites.length})</span>
              </h2>
              <div className="space-y-3">
                {invites.map((inv) => (
                  <div key={inv.id} className="rounded-xl bg-[#171A1D] border border-[#262B2F] p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-10 h-10 rounded-full bg-[#494F55]/30 text-[#8A9196] flex items-center justify-center shrink-0">
                          <Mail className="w-4 h-4" />
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-[#EDF0F1] truncate">{inv.email}</p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleResend(inv)} className="p-1.5 rounded-md text-[#8A9196] hover:text-[#D4AF37] hover:bg-[#494F55]/30 transition" title="Resend">
                          <Mail className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleCancelInvite(inv)} className="p-1.5 rounded-md text-[#8A9196] hover:text-red-400 hover:bg-red-500/10 transition" title="Cancel">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setInviteModal(false)} className="px-4 py-2 rounded-lg text-sm font-medium text-[#8A9196] hover:text-[#EDF0F1] hover:bg-[#494F55]/30 transition">Cancel</button>
            <button onClick={submitInvite} disabled={submitting} className="px-4 py-2 rounded-lg text-sm font-semibold text-[#1E252B] bg-[#D4AF37] hover:bg-[#c4a030] disabled:opacity-60 transition">
      ...
      >
        <form onSubmit={submitInvite} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#8A9196] mb-1.5 uppercase tracking-wider">Email Address</label>
            <input
              type="email"
              value={iForm.email}
          ...
          <div>
            <label className="block text-xs font-medium text-[#8A9196] mb-1.5 uppercase tracking-wider">Role</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[#8A9196] mb-1.5 uppercase tracking-wider">Permissions</label>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg border text-sm transition text-left ${
                      checked
                        ? 'bg-[#D4AF37]/10 border-[#D4AF37]/40 text-[#EDF0F1]'
                        : 'bg-[#1E252B] border-[#494F55]/40 text-[#8A9196] hover:border-[#494F55]/60'
                    }`}
                  >
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setRemoveTarget(null)} className="px-4 py-2 rounded-lg text-sm font-medium text-[#8A9196] hover:text-[#EDF0F1] hover:bg-[#494F55]/30 transition">Cancel</button>
      ...
      >
        <p className="text-sm text-[#8A9196]">
          Remove <span className="font-semibold text-[#EDF0F1]">{removeTarget?.name || removeTarget?.email}</span> from the team? They will lose all access immediately.
        </p>
      </Modal>
    </div>
  );
