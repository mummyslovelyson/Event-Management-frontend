...
export default function CategoriesPage() {
  ...
  const [form, setForm] = useState({ name: '', slug: '', icon: 'Layers', is_active: true });
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ name: '', slug: '', icon: 'Layers', is_active: true });
    setShowModal(true);
  };

  const openEdit = (cat) => {
    setEditing(cat);
    setForm({ name: cat.name, slug: cat.slug, icon: cat.icon || 'Layers', is_active: cat.is_active ?? cat.isActive ?? true });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { name: form.name, slug: form.slug || slugify(form.name), icon: form.icon, is_active: form.is_active ? 1 : 0 };
      if (editing) {
  ...
  const toggleActive = async (cat) => {
    try {
      const newVal = cat.is_active === 0 || cat.is_active === false ? 1 : 0;
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <button
              ...
              className={`relative w-10 h-5 rounded-full transition-colors ${form.is_active ? 'bg-white' : 'bg-[#494F55]/40'}`}
            >
